...
const BACKUP_ENDPOINT = '/api/backups/backup'
...
export const backupService = {
  ...
  async testConnection(): Promise<boolean> {
    try {
      const response = await fetch('/api/backups/health')
      return response.ok
    } catch (error) {
  ...
  async getBackups(): Promise<BackupMetadata[]> {
    try {
      const deviceId = this.getDeviceId()
      const response = await fetch(`/api/backups/backups/${deviceId}`)
  ...
  async findBackupsByName(name: string): Promise<BackupMetadata[]> {
    ...
    try {
      const response = await fetch(`/api/backups/find-backups?name=${encodeURIComponent(name)}`)
  ...
  async restoreBackup(backupId: string): Promise<boolean> {
    try {
      ...
      const response = await fetch(`/api/backups/backup/${encodeURIComponent(backupId)}`)
