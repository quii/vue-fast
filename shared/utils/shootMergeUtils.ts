 ...
 */
export function mergeShootChanges(target: Shoot, source: Shoot): void {
  ...
  if (source.lastUpdated.getTime() > target.lastUpdated.getTime()) {
    ...
    if (source.participants.length < target.participants.length) {
      ...
      target.participants = target.participants.filter(p =>
        sourceParticipantIds.includes(p.id)
      );
    }
  }
 ...
 */
export function deepCloneWithDates(shoot: Shoot): Shoot {
